...
torque_file = "Rhode Rage.xlsx"
intake_file = "speed_sweep.xlsx"
...
for length in intake_lengths:
    # Retrieve the data for the specific intake runner length
    filtered_data = df_intake[df_intake['INTAKE_RUNNER_LENGTH'] == length]
    speeds = filtered_data['SPEED'].values
    torques = filtered_data['TORQUE'].values

    # Create a copy of the Rhode Rage excel file and edit the Torque Curve sheet
    new_torque_file = os.path.join(output_folder, f"Rhode_Rage_{length}.xlsx")
    ...
    with pd.ExcelWriter(new_torque_file, engine='xlsxwriter') as writer:
        ...
        torque_curve_data = pd.DataFrame({
            ...
            'Torque [Nm, adjusted 0.7/0.9]' : torques * 7/9,
            'Torque [Nm]': torques
        })
