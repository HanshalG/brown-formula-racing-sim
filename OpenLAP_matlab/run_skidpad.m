function skidpad_time_out = run_skidpad(vehicle_name, display_graph)
    ...
    max_lat_acc = max(sim.lat_acc.data);
    skidpad_time_out = calculate_skidpad_time(max_lat_acc);
    
    %% Displaying laptime
    
    disp(['Laptime:  ',num2str(sim.laptime.data,'%3.3f'),' [s]'])
    output = sim.laptime.data
    ...
    if display_graph
        ...
        plot(tr.x,sim.sum_acc.data ./9.81,'k:')
